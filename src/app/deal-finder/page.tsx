...
const scanSteps = [
  { label: "Connecting to 12 Belgian data sources...", duration: 1000 },
  { label: "Scanning cadastre + omgevingsloket...", duration: 1500 },
  { label: "Parsing notarial announcements (staatsblad)...", duration: 1000 },
  { label: "Enriching ownership via KBO / BCE...", duration: 1000 },
  { label: "AI scoring against Straco acquisition criteria...", duration: 1500 },
]

const initialActivityFeed = [
  { time: "3 min ago", text: "2 new permits detected at omgevingsloket (Antwerpen)", type: "new" },
  { time: "18 min ago", text: "KBO enrichment complete — NV Vanderstraeten Holdings", type: "update" },
  { time: "42 min ago", text: "AI scored DEAL-BE-006 at 9.1/10 — BROWNFIELD, CURATOR SALE", type: "alert" },
  { time: "1 hr ago", text: "Notarial announcement filed — Estate De Backer (Gent)", type: "new" },
  { time: "2 hr ago", text: "Cadastre refresh — Oost-Vlaanderen", type: "update" },
  { time: "3 hr ago", text: "New Bodemdecreet flag — Scheldestraat, Antwerpen", type: "alert" },
]
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Off-Market Deal Sourcing</h1>
            <p className="text-sm text-gray-400 flex items-center gap-2 mt-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              {dashboardStats.sourcesActive} Belgian data sources active &middot; Last scan: {new Date(dashboardStats.lastScanTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              <span className="text-gray-300">|</span>
              {dashboardStats.countiesMonitored} regions monitored
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                scanning ? "bg-[#1A1A2E] text-white animate-pulse cursor-wait"
                : scanComplete ? "bg-emerald-500 text-white cursor-default"
                : "bg-[#1A1A2E] text-white hover:bg-[#0F0F1E] shadow-lg shadow-[#1A1A2E]/25"
              }`}
            >
        ...
        <AnimatePresence>
          {scanning && (
            ...
            >
              <div className="bg-[#1A1A2E]/5 border border-[#1A1A2E]/20 rounded-xl p-4">
                ...
                <div className="flex items-center gap-4 mt-2">
                  {scanSteps.map((_, i) => (
                    <span key={i} className={`w-2 h-2 rounded-full ${i < scanStep ? "bg-emerald-500" : i === scanStep ? "bg-[#1A1A2E] animate-pulse" : "bg-gray-300"}`} />
                  ))}
                </div>
              </div>
            </motion.div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { label: "Total Deals", value: totalDeals.toString(), sub: `${scanComplete ? dashboardStats.newThisWeek + hiddenDeals.length : dashboardStats.newThisWeek} new this week`, icon: Search, color: "text-[#1A1A2E]" },
          ...
          <div className="lg:col-span-3 space-y-3">
            ...
            {filteredDeals.length === 0 && (
              <div className="bg-white border border-dashed border-gray-300 rounded-xl p-12 text-center">
                ...
                <button onClick={() => setCountyFilter("all")} className="mt-3 text-xs text-[#1A1A2E] font-medium hover:underline">Clear filters</button>
              </div>
            )}
              ...
              return (
                <motion.div
                  ...
                  className={`group bg-white border rounded-xl p-4 cursor-pointer transition-all hover:shadow-md ${
                    isNew ? "border-[#1A1A2E] ring-1 ring-[#1A1A2E]/20" : "border-gray-200 hover:border-gray-300"
                  }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {isNew && (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#1A1A2E] text-white animate-pulse">
                            NEW
                          </span>
                        )}
                      </div>
                      <h3 className="text-base font-bold text-gray-900 group-hover:text-[#1A1A2E] transition-colors">{deal.address}</h3>
                    ...
                    <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-[#1A1A2E] transition-colors mt-2 flex-shrink-0" />
                  </div>
                ...
                {dashboardStats.countyBreakdown.map(c => (
                  <div key={c.county}>
                    ...
                    <div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden">
                      <div className="h-full rounded-full bg-[#1A1A2E]/50" style={{ width: `${(c.deals / 12) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
