...
const stageConfig: Record<PipelineStage, { label: string; color: string; bgColor: string }> = {
  new: { label: "Permitting", color: "text-blue-600", bgColor: "bg-blue-50 border-blue-200" },
  skip_traced: { label: "Preparation", color: "text-purple-600", bgColor: "bg-purple-50 border-purple-200" },
  sequence_active: { label: "Construction / Sales", color: "text-amber-600", bgColor: "bg-amber-50 border-amber-200" },
  replied: { label: "Near Completion", color: "text-emerald-600", bgColor: "bg-emerald-50 border-emerald-200" },
  meeting_set: { label: "Delivered / Stabilized", color: "text-[#1A1A2E]", bgColor: "bg-blue-50 border-[#1A1A2E]/30" },
}
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Portfolio Command Center</h1>
            <p className="text-sm text-gray-400 mt-1">25 development projects &middot; 120 investment properties &middot; weekly AI intelligence.</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
          {[
            { label: "Active Projects", value: outreachStats.totalLeads.toString(), icon: Users, color: "text-[#1A1A2E]" },
            { label: "Data sync health", value: `${outreachStats.skipTraceHitRate}%`, icon: Target, color: "text-purple-500" },
            { label: "On schedule", value: `${outreachStats.openRate}%`, icon: Eye, color: "text-amber-500" },
            { label: "Flagged items", value: `${outreachStats.replyRate}%`, icon: MessageSquare, color: "text-emerald-500" },
            { label: "Milestones this week", value: outreachStats.meetingsSet.toString(), icon: Calendar, color: "text-blue-500" },
            { label: "AI briefs generated", value: outreachStats.sequencesActive.toString(), icon: Send, color: "text-orange-500" },
          ].map(s => (
                    ...
                    {leads.map(lead => (
                      <motion.div
                        ...
                        className={`bg-white border rounded-xl p-3 cursor-pointer hover:shadow-md transition-all ${
                          selectedLead?.id === lead.id ? "border-[#1A1A2E] shadow-md" : "border-gray-200"
                        }`}
                      >
                  ...
                  {selectedLead.meetingDate && (
                    <div className="bg-[#1A1A2E]/5 border border-[#1A1A2E]/20 rounded-xl p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-[#1A1A2E]">Meeting Scheduled</h4>
                      ...
                      {emailSequence.map((email, i) => {
                        ...
                        return (
                          <div key={i} className={`border rounded-xl p-4 transition-all ${
                            isCurrent ? "border-[#1A1A2E] bg-[#1A1A2E]/5" : isActive ? "border-emerald-200 bg-emerald-50/50" : "border-gray-200 bg-gray-50 opacity-60"
                          }`}>
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
